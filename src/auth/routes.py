...
from .schemas import EmailModel,UserCreateModel, UserModel, UserLoginModel, UserBooksModel, PasswordResetRequestModel, PasswordResetConfirmModel
from .service import UserService
from src.db.main import get_session
from sqlmodel.ext.asyncio.session import AsyncSession
from  fastapi.exceptions import HTTPException
from .utils import create_access_token, create_url_safe_token, decode_token, generate_passwd_hash, verify_password, decode_url_safe_token
...
async def login_users(login_data: UserLoginModel, session: AsyncSession = Depends(get_session)):
    ...
    if user is not None:
        password_valid = verify_password(password, user.password_hashed)
...
async def reset_account_password(
    ...
    if user_email:
        ...
        await user_service.update_user(user, {"password_hashed": passwd_hash}, session)
